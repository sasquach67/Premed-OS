...
export const UNIT_QUESTION_BANK_V1: ArtifactSpec = {
  ...
  rules: [
    ...
    { id: 'UQB-TEXTBOOK-VISION', kind: 'invariant', text: 'When visually readable textbook evidence is supplied, use the lecture objectives, transcript, and assigned questions to select only textbook figures that directly clarify the closed lesson scope. Ignore decorative, tangential, and redundant figures. Never claim a figure was interpreted when only OCR or caption text was available.' },
    { id: 'UQB-WEB-PATTERN', kind: 'invariant', text: 'Before authoring each bank, Claude must inspect current official public assessment examples on the web for cognitive patterns, experiment structures, graph use, and distractor logic. Web material may shape the assessment pattern only; never copy or import its question wording, images, values, or biological claims.' },
    ...
    { id: 'UQB-PDF-READY', kind: 'invariant', text: 'Keep visual labels, captions, tables, and answer explanations concise enough to render legibly in a printable PDF. The answer key must remain separable from the student question section.' },
