...
1. Establish the actual assessment scope and known format from the supplied instructions/review sheet and explicit student request. Preserve qualifiers, exclusions and each subrequirement, including response format, required methods and length constraints when supplied. Put these in the requirements ledger even when entry.request.assessmentFormat repeats a known format. If unknown, state that once and prepare selected material without inventing exam coverage.
