...
import { learningVisualItems } from './learningVisuals'
...
export function notebookEntryAssetIds(pkg: PortableNotebookPackage, entryId: string): Set<string> {
  ...
  for (const item of [...sections.flatMap(s => s.blocks), ...entry.requirements, ...entry.objectives] as (VisualNotebookBlock | { assetIds?: string[] })[]) {
    ...
    if ('type' in item) for (const nested of learningVisualItems(item)) for (const id of visualAssetReferences(nested)) ids.add(id)
  }
  if (pkg.version !== 2) {
...
export function notebookVisualPracticeKey(pkg: PortableNotebookPackage, entryId: string, practiceId: string, bindings: readonly NotebookAssetBinding[] = []): string {
  ...
  if (!question || (question.type !== 'practice' && question.type !== 'worked-example')) return canonical({ missingPractice: practiceId })
  ...
  for (const stimulus of stimuli) if ('type' in stimulus) {
    ...
    for (const item of learningVisualItems(stimulus)) for (const id of visualAssetReferences(item)) ids.add(id)
  }
  for (const item of learningVisualItems(question)) for (const id of visualAssetReferences(item)) ids.add(id)
  const assets = pkg.version !== 2 ? pkg.assets : [], pending = [...ids]
