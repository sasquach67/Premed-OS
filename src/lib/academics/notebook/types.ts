...
export type Evidence = { sourceIds: string[]; excerptIds: string[] }
export type NotebookBlock = Evidence & { id: string; provenance: 'source' | 'clarification' | 'background' | 'generated-practice' | 'student-work' } & (
  ...
  { type: 'table'; columns: string[]; rows: string[][] } | { type: 'practice'; prompt: string; answer: string; rationale: string } |
...
export type NotebookPackage = { format: 'premed-os-notebook-package'; version: 2; instructionsVersion: 'notebook-workflows-draft-2'; course: { code: string; title: string; term: string | null }; sources: NotebookSource[]; entries: NotebookEntry[] }
export type NotebookProgress = Record<string, { response: string; complete: boolean }>
export type ImportedNotebook = { original: NotebookPackage; current: NotebookPackage; originalRaw: string; entryId: string; fingerprint: string; importedAt: number; editedAt?: number; progress: NotebookProgress; notes: string; revisedFromLectureId?: string }
