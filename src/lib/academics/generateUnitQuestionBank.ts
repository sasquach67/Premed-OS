...
export async function generateUnitQuestionBank({
  ...
  const assembled = assembleGenerationRequest({
    ...
    request: [
      ...
      privateAssessmentPhrases.length ? 'Use assessment moves only; do not reproduce private assessment wording.' : '',
    ].filter(Boolean).join('\n'),
  })
  const result = await studyTools.generate({ action: 'generate', courseId, topicId: prepared.scopeId, chunkIds: assembled.chunkIds, specId: assembled.specId, specHash: assembled.specHash, systemPrompt: assembled.systemPrompt, request: `Unit: ${unit}. Build the source-grounded unit question bank.` })
  if (!result.ok) return { ok: false, failure: failureFor(result.code), message: result.message }
  const artifact = validateUnitQuestionBank(result.data.artifact, assembled.chunkIds, privateAssessmentPhrases, masteryStandardIds)
