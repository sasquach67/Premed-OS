...
.type { font-family: ui-monospace, "SF Mono", Menlo, Consolas, monospace; font-size: 12px; letter-spacing: .08em; margin-bottom: 18px; }
.verb { color: var(--ac); font-weight: 700; }.verb::before { content: "■ "; font-size: 9px; vertical-align: 2px; margin-right: 2px; }
.tname { color: #8a8172; margin-left: 8px; }.tname::before { content: "/  "; }
hr#answer { border: none; border-top: 1px solid #e3d9c6; margin: 22px 0; }
.answer { font-weight: 700; }
.cloze { color: var(--ac); font-weight: 700; }
ol.blurt { margin: 0; padding-left: 0; list-style: none; counter-reset: b; } ol.blurt li { counter-increment: b; position: relative; padding-left: 32px; margin: 0 0 12px; font-size: 18px; line-height: 1.5; } ol.blurt li::before { content: counter(b); position: absolute; left: 0; top: 2px; font-family: ui-monospace, "SF Mono", Menlo, Consolas, monospace; font-size: 13px; font-weight: 700; color: var(--ac); }
...
.nightMode .extra-label, .night_mode .extra-label { color: #7d7362; }.nightMode .tname, .night_mode .tname { color: #8d847a; }`
...
function typeLabel(type: GeneratedFlashcard['type']) {
  const labels: Record<GeneratedFlashcard['type'], [string, string]> = {
    basic: ['say it', 'recall'], cloze: ['say it', 'definition'], conceptual: ['explain it', 'conceptual'], process: ['say it', 'process'], comparison: ['connect it', 'comparison'], application: ['use it', 'application'], exemplar: ['connect it', 'example → concept'], 'free-recall': ['blurt it', 'free recall'],
  }
  const [verb, name] = labels[type]
  return `<span class="verb">${verb}</span><span class="tname">${name}</span>`
}

function mindsetFor(type: GeneratedFlashcard['type']) {
  if (type === 'comparison' || type === 'exemplar') return 'm-connect'
  if (type === 'application') return 'm-use'
  if (type === 'free-recall') return 'm-blurt'
  if (type === 'conceptual') return 'm-explain'
  return 'm-say'
}

function deckRecords(name: string, nowSeconds: number) {
  const parts = name.split('::').filter(Boolean)
  const result: Record<number, Record<string, unknown>> = {}
  for (let index = 0; index < parts.length; index += 1) {
    const id = DEFAULT_DECK_ID + index
    result[id] = { id, name: parts.slice(0, index + 1).join('::'), mod: nowSeconds, usn: -1, desc: index === parts.length - 1 ? 'Generated by premedOS. Review and scheduling live in Anki; premedOS never reads this deck back.' : '', dyn: 0, collapsed: false, browserCollapsed: false, newToday: [0, 0], revToday: [0, 0], lrnToday: [0, 0], timeToday: [0, 0], conf: 1, extendNew: 0, extendRev: 0 }
  }
  return result
}

function exportTags(card: GeneratedFlashcard) {
  const type = card.type.toUpperCase().replace('-', '_')
  const mechanism = card.clozePattern ?? 'basic'
  const tags = [
    `premedos::type::${type}`,
    `premedos::mechanism::${mechanism}`,
    `premedos::mindset::${mindsetFor(card.type).slice(2)}`,
    `premedos::salience::${card.salience ?? 'unknown'}`,
    `premedos::difficulty::${card.difficultyEstimate ?? 'unknown'}`,
    'premedos::preset::premedos-default',
    'premedos::mode::SOURCE_PLUS_CLARIFICATION',
    `premedos::concept::${card.conceptId}`,
    ...card.tags,
  ]
  return [...new Set(tags)].map((tag) => tag.replaceAll(' ', '_')).join(' ')
}

function backFor(card: GeneratedFlashcard) {
  if (card.type !== 'free-recall') return ankiText(card.back)
  return `<ol class="blurt">${(card.recallItems ?? []).map((item) => `<li>${ankiText(item)}</li>`).join('')}</ol>`
}
...
export async function flashcardApkg(deck: GeneratedFlashcardDeck): Promise<Uint8Array> {
  ...
  const basic = ankiModel(BASIC_MODEL_ID, 'premedOS Basic', 0, ['Front', 'Back', 'Extra', ...commonFields], [{ name: 'Card 1', qfmt: '<div class="pm {{Mindset}}"><div class="type">{{Type}}</div>{{Front}}</div>', afmt: '<div class="pm {{Mindset}}"><div class="type">{{Type}}</div>{{Front}}<hr id="answer"><div class="answer">{{Back}}</div>{{#Extra}}<div class="extra"><span class="extra-label">Extra</span>{{Extra}}</div>{{/Extra}}</div>' }], nowSeconds)
  const cloze = ankiModel(CLOZE_MODEL_ID, 'premedOS Cloze', 1, ['Text', 'Extra', ...commonFields], [{ name: 'Cloze', qfmt: '<div class="pm {{Mindset}}"><div class="type">{{Type}}</div>{{cloze:Text}}</div>', afmt: '<div class="pm {{Mindset}}"><div class="type">{{Type}}</div>{{cloze:Text}}{{#Extra}}<div class="extra"><span class="extra-label">Extra</span>{{Extra}}</div>{{/Extra}}</div>' }], nowSeconds)
  const decks = deckRecords(deck.title, nowSeconds)
  const activeDeckId = DEFAULT_DECK_ID + Math.max(0, deck.title.split('::').filter(Boolean).length - 1)
  ...
  db.run(
    ...
    [1, nowSeconds, nowSeconds, nowMs, 11, 0, -1, 0, JSON.stringify({ nextPos: deck.cards.length + 1, estTimes: true, activeDecks: [activeDeckId], curDeck: activeDeckId, curModel: BASIC_MODEL_ID, newSpread: 0, collapseTime: 1200 }), JSON.stringify({ [BASIC_MODEL_ID]: basic, [CLOZE_MODEL_ID]: cloze }), JSON.stringify(decks), JSON.stringify({ 1: deckConfig }), '{}'],
  )
  ...
  deck.cards.forEach((card, index) => {
    ...
    const details = [typeLabel(card.type), mindsetFor(card.type), ankiText(card.conceptId), ankiText(`Material chunk ${card.sourceChunkId}`), ankiText(`${deck.specId}@${deck.specHash}`)]
    const fields = isCloze
      ? [ankiText(card.cloze), ankiText(card.extra), ...details].join(FIELD_SEPARATOR)
      : [ankiText(card.front), backFor(card), ankiText(card.extra), ...details].join(FIELD_SEPARATOR)
    const sortField = isCloze ? ankiText(card.cloze) : ankiText(card.front)
    const tags = exportTags(card)
    db.run(
    ...
    db.run(
      ...
      [cardId, noteId, activeDeckId, 0, nowSeconds, -1, 0, 0, index + 1, 0, 0, 0, 0, 0, 0, 0, 0, ''],
    )
  })
