...
export async function generateFullMock({ courseId, chunks, label }: { courseId: string; topicId?: string; chunks: SourceChunk[]; label: string }): Promise<FullMockGenerationOutcome> {
  ...
  const assembled = assembleGenerationRequest({ specId: 'class-full-mock-v1', chunkIds: prepared.chunkIds, request: `Exam: ${label}. Action: generate a timed class practice mock from these sources only.` })
  const result = await studyTools.generate({ action: 'generate', courseId, topicId: prepared.scopeId, chunkIds: assembled.chunkIds, specId: assembled.specId, specHash: assembled.specHash, systemPrompt: assembled.systemPrompt, request: `Exam: ${label}.` })
