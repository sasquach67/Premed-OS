...
describe('site-wide demo data', () => {
  ...
  it('seeds every locked interesting state', () => {
    ...
    expect(center.lectures.filter((lecture) => lecture.courseId === data.courses.find((course) => course.code === 'CHEM 262')?.id)).toHaveLength(3)
    expect(center.lectures.some((lecture) => lecture.courseId === data.courses.find((course) => course.code === 'ENGL 105')?.id && lecture.transcriptFileId)).toBe(true)
    expect(center.topics.filter((topic) => data.courses.find((course) => course.id === topic.courseId)?.status === 'in-progress').every((topic) => Boolean(topic.scheduledFor))).toBe(true)
    expect(center.professorEvidence).toHaveLength(2)
    expect(center.transcriptRecords).toHaveLength(8)
    expect(center.transcriptRecords.every((record) => record.institution === 'UNC Chapel Hill' && record.courseType === 'regular')).toBe(true)
    expect(center.termReports.some((report) => report.status === 'ready' && report.term === 'Spring 2026')).toBe(true)
    expect(center.retrievabilityPredictions).toHaveLength(0)
    expect(center.plannerTerms.map((term) => term.label)).toEqual(['Spring 2026', 'Fall 2026', 'Spring 2027', 'Fall 2027'])
    expect(center.plannerTerms.filter((term) => !term.lockedAt).every((term) => data.courses.some((course) => course.plannerTermId === term.id))).toBe(true)
