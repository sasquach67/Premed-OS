...
export function JournalEntryPage() {
  const [flashcardEntryId, setFlashcardEntryId] = useState<string | null>(null)
  ...
  return <section className="notebook-entry-page w-full min-w-0 space-y-5 px-4 pb-8 pt-2 md:px-8">
    ...
    {entry && <div className="flex justify-end"><Button variant="outline" onClick={() => setFlashcardEntryId(entry.id)}><Brain aria-hidden="true" />Create flashcards</Button></div>}
    {entry && flashcardEntryId === entry.id && <FlashcardPromptPanel key={`flashcards-${entry.id}`} courseId={course.id} courseLabel={course.code} lectureId={entry.id} onClose={() => setFlashcardEntryId(null)} />}
