...
const drive = vi.hoisted(() => ({ uploadBackup: vi.fn(async () => 'synthetic-file'), isConnected: () => true, connect: vi.fn(), connectSilent: vi.fn(), disconnect: vi.fn(), clearDriveSession: vi.fn(), downloadBackup: vi.fn() }))
...
beforeEach(() => { localStorage.clear(); useStore.persist.setOptions({ name: 'hq:app-data:guest' }); activateGuestWorkspace(); drive.uploadBackup.mockReset().mockResolvedValue('synthetic-file'); drive.connectSilent.mockReset().mockResolvedValue(undefined); root = createRoot(document.createElement('div')) })
...
it('does not write A backup metadata into B after an awaited upload', async () => {
  ...
  drive.uploadBackup.mockImplementationOnce(() => new Promise(r => { finish = r }))
...
it('waits for verified account readiness before the daily backup and then resumes it', async () => {
  ...
  function Probe() { backup = useBackup(); return null }
  await act(async () => root.render(createElement(Probe)))
  expect(drive.connectSilent).not.toHaveBeenCalled(); expect(drive.uploadBackup).not.toHaveBeenCalled()
  await act(async () => allowAccountSync(token))
  expect(drive.connectSilent).toHaveBeenCalledTimes(1)
  expect(drive.uploadBackup).toHaveBeenCalledTimes(1)
