...
import { revisionInput, notebookUpdateBaselineFilename } from '@/lib/academics/notebook/revision'
  ...
  return <section className="external-notebook en-flow" aria-label="External AI notebook workflow">
    ...
    {step === 'handoff' && <div className="en-stage-content">
      ...
      <ol className="en-handoff-list">
        ...
        <li><span className="en-task-number" aria-hidden="true">2</span><div><h2>{revision ? 'Attach the saved notebook, images and new materials' : 'Attach your materials'}</h2><p className="en-text">{values.MATERIALS || goalInfo.bring}</p><p className="en-muted">{revision ? 'Attach the matching saved notebook JSON and its existing referenced images, plus new or revised notes, images or readings. A whole revised file is fine; your AI should compare repeated content, additions and corrections. Reattach older originals only where retained evidence lacks needed context. ' : 'Upload the originals in your AI. '}An upload, connection or retrieved excerpt does not prove every file was read. Ask what was inspected and what remains unread.</p><p className="en-muted">Use supported individual files, direct images or pasted text. For unclear scans, handwriting or embedded figures, add clear page images or crops and type unclear text or formulas. Keep the full question, options and diagram together. Batch by topic when needed; projects and connections are optional.</p>{goal === 'review' && <p className="en-muted">Use a recording only if your AI can inspect it. Otherwise, use a readable transcript.</p>}</div></li>
        <li><span className="en-task-number" aria-hidden="true">3</span><div><h2>Get the notebook JSON and images</h2><p>Your AI should create the complete notebook JSON directly after checking the sources and content. A readable draft or discussion is optional if you ask for it; no separate approval message is required. Download the <strong>.json file</strong> named after the notebook's title. If file downloads are unavailable, get the complete JSON block.</p><p>Before leaving your AI, download every referenced original PNG/JPEG alongside the JSON, keeping the matching filenames. Filenames in JSON do not contain the image files. A text-only notebook needs only its JSON.</p><p className="en-muted">Browser settings control where downloads are saved; Premed OS cannot force the Downloads folder.</p><p className="en-muted">If an image download is missing or has expired, ask for the downloadable original again or recover the exact original file from your materials. Do not substitute a different image under the same ID. If a source image genuinely cannot be obtained, ask your AI to record it as unavailable with an actionable next step and revise dependent content rather than declaring a file you do not have. Premed OS cannot save a notebook with missing declared images.</p></div></li>
      </ol>
      <details className="en-brief-note">
        <summary>What to expect from your AI</summary>
        <p>{conversationExpectations.shortCopy}</p>
        <p>Lots of files? Send them in batches. If you say more files are coming, your AI should wait until you say you are done uploading before generating the JSON.</p>
        <p>{goalInfo.limit}</p>
      ...
      <details className="en-small-detail"><summary>Before you leave this page</summary>
        <p>Your step and inputs are kept per class in this browser tab when storage is available. Closing the tab can lose this draft; keep your downloaded prompt, notebook JSON and every referenced image file. Remembered class preferences are saved separately.</p>
      ...
      <div className="en-gate">
        <div><p className="en-gate-title">Do you have the notebook JSON and its referenced images?</p><p className="en-muted">This is your confirmation. Premed OS cannot check what happened in your AI.</p></div>
        <Button variant={draft.jsonReady ? 'outline' : 'default'} disabled={baselineBlocked} aria-pressed={draft.jsonReady} onClick={() => { if (!baselineBlocked) { setDraft(previous => ({ ...previous, jsonReady: true })); setMessage('Notebook files readiness noted. Click Next to validate and preview them here.') } }}>{draft.jsonReady && <Check aria-hidden="true" />}I have my notebook files</Button>
      </div>
      <footer className="en-stage-footer"><div className="en-actions"><Button variant={draft.jsonReady ? 'default' : 'outline'} disabled={!draft.jsonReady || baselineBlocked} onClick={() => goTo('import')}>Next<ArrowRight aria-hidden="true" /></Button><Button variant="ghost" onClick={() => goTo('prompt')}><ArrowLeft aria-hidden="true" />Back to prompt</Button></div>
        <p className="en-next-note">Materials go to your AI. The finished notebook JSON and its referenced PNG/JPEG files come back to Premed OS.</p>
      </footer>
    </div>}
