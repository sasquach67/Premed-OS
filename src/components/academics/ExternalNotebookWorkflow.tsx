  ...
  return <section className="external-notebook en-flow" aria-label="External AI notebook workflow">
    ...
    {step === 'handoff' && <div className="en-stage-content">
      ...
      <ol className="en-handoff-list">
        ...
        <li><span className="en-task-number" aria-hidden="true">3</span><div><h2>Review, then get the notebook and images</h2><p>Ask questions or request changes to the content. After changes, review the updated draft and a short account of what changed before approving the JSON. Ask for a downloadable <strong>.json file</strong> containing the complete approved notebook. If JSON downloads are unavailable, ask for the complete JSON block.</p><p>Before leaving your AI, download every referenced original PNG/JPEG alongside the JSON, keeping the matching filenames. Filenames in JSON do not contain the image files. A text-only notebook needs only its JSON.</p><p className="en-muted">If an image download is missing or has expired, ask for the downloadable original again or recover the exact original file from your materials. Do not substitute a different image under the same ID. If a source image genuinely cannot be obtained, ask your AI to record it as unavailable with an actionable next step and revise dependent content rather than declaring a file you do not have. Premed OS cannot save a notebook with missing declared images.</p></div></li>
      </ol>
