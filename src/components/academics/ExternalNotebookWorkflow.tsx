  ...
  return <section className="external-notebook en-flow" aria-label="External AI notebook workflow">
    ...
    {step === 'handoff' && <div className="en-stage-content">
      ...
      <ol className="en-handoff-list">
        ...
        <li><span className="en-task-number" aria-hidden="true">3</span><div><h2>Download your notebook</h2><p>Download the <strong>notebook ZIP</strong> with its JSON and images. Extract it to a folder, then choose that folder on the import screen.</p></div></li>
      </ol>
      <details className="en-brief-note en-handoff-help">
        ...
        <dl>
          ...
          <div><dt>Recordings</dt><dd>Use a recording only if your AI can inspect it. Otherwise, provide a readable transcript.</dd></div>
          <div><dt>Missing images</dt><dd>Download every referenced original PNG/JPEG with the JSON. Filenames alone do not contain the image files. If a download is missing or expired, ask for the downloadable original again or recover the exact original file. Do not substitute a different image under the same ID. If it cannot be obtained, ask your AI to record it as unavailable with an actionable next step and revise dependent content. Missing declared images prevent saving.</dd></div>
          <div><dt>Finding your downloads</dt><dd>Check your browser’s downloads. Prefer one ZIP with the notebook JSON and every required image. Extract it and choose the complete folder, or use Other import options to import the ZIP. If ZIP creation is unavailable, keep the complete folder or downloadable file set together. If your AI cannot create files, use Paste JSON instead and add the actual required images. Raw lecture files alone cannot create a notebook.</dd></div>
          <div><dt>Coming back later</dt><dd>Your step and inputs stay in this tab when storage is available. Closing it can lose the draft, so keep your prompt, JSON and images. {revision ? 'Reopen this saved notebook and choose Update this notebook.' : 'Open Class notebook and choose Add to notebook.'} Remembered class preferences are saved separately.</dd></div>
          {revision && <div><dt>Updating a notebook</dt><dd>A new chat is fine. Add the matching saved baseline and existing images. Reattach older originals where the saved evidence lacks needed context.</dd></div>}
        </dl>
      </details>
