...
export function GeneratedLectureGuideView({ lecture, guide, chunks, files, mastery, onOpenMastery }: { lecture: LectureRecord; guide: StudyGuideArtifact; brief: NonNullable<LectureRecord['lectureBrief']>; chunks: SourceChunk[]; files: AcademicFile[]; mastery?: Outline; onOpenMastery: () => void }) {
  ...
  return <div className="@container mx-auto max-w-6xl">
    ...
    <div className="mt-6 grid items-start gap-8 @min-[52rem]:grid-cols-[13rem_minmax(0,1fr)] @min-[52rem]:gap-10">
      ...
      <article className="min-w-0 rounded-2xl border border-border bg-card px-5 py-6 shadow-sm sm:px-9 sm:py-8">
...
function MasterySection({ label, items }: { label: string; items: string[] }) { return items.length ? <section><h4 className="text-sm font-extrabold">{label}</h4><ul className="mt-2 list-disc space-y-2 pl-5 text-sm leading-7 marker:text-primary">{items.map((item, index) => <li key={index}>{item}</li>)}</ul></section> : null }
