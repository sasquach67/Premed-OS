...
function GpaView({ ledger }: { ledger: ReturnType<typeof buildGradeLedger> }) {
  ...
  const deltaText = ledger.delta == null ? 'Both calculations need eligible records before a difference can be shown. UNC uses completed in-residence courses; AMCAS uses the saved transcript attempts. Review missing details in the ledger.' : Math.abs(ledger.delta) < .005 ? 'The recorded local and AMCAS preview currently agree.' : `The difference reflects the records each calculation can currently support — review all institutions, attempts, and classification evidence.`
