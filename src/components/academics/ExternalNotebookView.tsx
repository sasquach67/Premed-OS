...
function ReaderSources({ blocks, pkg, scope, inlineSources, onInlineSources, selectedBlock, panelRef }: { blocks: NotebookBlock[]; pkg: NotebookPackage; scope: 'section' | 'item'; inlineSources: boolean; onInlineSources: () => void; selectedBlock?: string | null; panelRef?: RefObject<HTMLDetailsElement | null> }) {
  ...
  return <details ref={panelRef} className={`nbr-sources-panel ${scope === 'section' ? 'nbr-srcpanel' : 'nbr-srcpanel-practice'}`}>
    ...
    <div className="nbr-sources-body">
      ...
      {sources.map(({ source, excerpts }) => <section className="nbr-source" key={source.id}><h4>{source.title}</h4><p className="en-muted">{source.role} / {source.access} / {source.used ? 'Used' : 'Not used'}</p>{source.limitations.map((limit, index) => <p className="en-text" key={index}>{limit}</p>)}{!excerpts.length && <p className="en-muted">No linked excerpts</p>}{excerpts.map(excerpt => <blockquote key={excerpt.id} data-blocks={JSON.stringify(excerpt.blockIds)} data-highlight={Boolean(selectedBlock && excerpt.blockIds.includes(selectedBlock)) || undefined}><small>{excerpt.location ?? 'Location not supplied'}</small><p className="en-text">{excerpt.text}</p></blockquote>)}</section>)}
    </div>
  </details>
}
function BlockView({ block, path, change, pkg, progress, onProgress, reader = false, inlineSources = false, onShowEvidence }: { block: NotebookBlock; path: (string | number)[]; change?: ChangeText; pkg: NotebookPackage; progress?: NotebookProgress; onProgress?: (id: string, response: string, complete: boolean) => void; reader?: boolean; inlineSources?: boolean; onShowEvidence?: (id: string) => void }) {
  ...
  return <div className={`en-block en-block-${block.type}`}>
    ...
    {block.type === 'practice' && <><b>Try it yourself</b>{text(block.prompt, 'prompt', 'Practice prompt')}{onProgress && !change && <div><label className="en-field">Your response<textarea aria-label={`Your response to ${block.id}`} value={work.response} onChange={event => onProgress(block.id, event.target.value, work.complete)} /></label><label className="en-check"><input type="checkbox" checked={work.complete} onChange={event => onProgress(block.id, work.response, event.target.checked)} />I can explain this without looking</label></div>}<details className="en-answer nbr-reveal"><summary>Reveal answer and explanation</summary>{text(block.answer, 'answer', 'Answer')}{text(block.rationale, 'rationale', 'Explanation')}{reader ? <><small className="en-muted">{block.provenance.replaceAll('-', ' ')}</small><ReaderSources blocks={[block]} pkg={pkg} scope="item" inlineSources={practiceSources} onInlineSources={() => setPracticeSources(previous => !previous)} /></> : <EvidenceView evidence={block} pkg={pkg} />}</details></>}
...
export function NotebookPackageView({ pkg, entryId, change, progress, onProgress, mode = 'all', reader = false }: { pkg: NotebookPackage; entryId?: string; change?: ChangeText; progress?: NotebookProgress; onProgress?: (id: string, response: string, complete: boolean) => void; mode?: ReadingMode; reader?: boolean }) {
  const prefix = useId()
  const text = (value: string, path: (string | number)[], label: string) => <ContentText value={value} path={path} label={label} change={change} />
  ...
  const document = <div className={`external-notebook en-document${reader ? ' nbr-doc' : ''}`} data-reader-mode={reader ? mode : undefined}>{!reader && <p className="en-badge">Package class: {pkg.course.code} / {pkg.course.title}{pkg.course.term ? ` / ${pkg.course.term}` : ''}</p>}
    {pkg.entries.map((entry, ei) => (!entryId || entry.id === entryId) && <article key={entry.id} aria-label={entry.title}>
      ...
      {(mode === 'all' || mode === 'coverage') && <section aria-label="Requirement coverage"><h3 id={readerHeadingId(prefix, entry.id, 'coverage')} tabIndex={-1}>Requirement coverage</h3><div className="en-coverage">{(['supported', 'partial', 'missing', 'out-of-scope'] as const).map(status => <span key={status} data-status={status}>{status}: {entry.requirements.filter(r => r.status === status).length}</span>)}</div>
      ...
      {(!reader || mode === 'coverage' || mode === 'all') && entry.limitations.length > 0 && <section className="en-notice"><h3 id={readerHeadingId(prefix, entry.id, 'limits')} tabIndex={-1}>Limits of this entry</h3>{entry.limitations.map((item, i) => <div key={i}>{text(item, ['entries', ei, 'limitations', i], 'Entry limitation')}</div>)}</section>}
    </article>)}
...
export function ExternalNotebookView({ lecture, courseCode, onNavigateEntry }: { lecture: LectureRecord; courseCode: string; onNavigateEntry?: (id: string) => void }) {
  ...
  if (updating && n.updateSession) return <section className="external-notebook" aria-label="Update saved notebook"><div className="en-actions"><Button variant="outline" onClick={() => setUpdating(false)}>Back to saved entry</Button><Button variant="ghost" onClick={() => startUpdate(true)}>Restart from latest saved entry</Button></div><p className="en-muted">Restart only when you need a newer baseline. Keep any unfinished prompt or proposal first; it starts a fresh update draft, not a new notebook.</p><p role="status">{message}</p><ExternalNotebookWorkflow key={n.updateSession.id} courseId={lecture.courseId} revision={n.updateSession} baselineFresh={!staleUpdate} onImported={id => { setUpdating(false); setMessage(id === lecture.id ? 'Notebook update saved. Previous versions remain in history.' : 'Separate entry saved. The original notebook remains unchanged.'); onNavigateEntry?.(id) }} /></section>
  ...
  return <section className="external-notebook nbr" aria-label="Saved external notebook"><header className="en-header nbr-head"><div className="nbr-head-main"><p className="en-eyebrow">Saved in {courseCode}</p><h1>{lecture.title}</h1><p className="nbr-scope en-text">{entry.scope}</p><ul className="nbr-chips"><li>{entry.goal} / Revision {entry.revision}{entry.baseRevision ? ` / Based on revision ${entry.baseRevision}` : ''}</li><li>{entry.sections.length} sections</li><li>{practiceCount} practice items</li><li>{n.current.sources.length} sources</li>{n.editedAt && <li>Updated locally</li>}</ul></div><div className="en-actions nbr-head-actions">{(practiceCount > 0 || entry.objectives.length > 0) && <Button onClick={() => setMode('practice')}>Practice recall</Button>}<Button variant="outline" onClick={() => startUpdate()}>Update this notebook</Button></div></header>
    <details className="nbr-tools"><summary>Entry tools</summary><div className="nbr-tools-body"><div className="en-actions"><Button variant="outline" disabled={Boolean(draft)} onClick={() => { setDraft(structuredClone(n.current)); setEditBase(notebookContentKey(n)); setMessage('Editing a separate copy. Save to keep changes; the previous content stays in history.') }}>Edit entry</Button></div>
    ...
    <nav className="en-actions nbr-views" aria-label="Notebook reading views">{(['study', 'practice', 'coverage', 'sources'] as const).map(view => <Button key={view} variant={mode === view ? 'default' : 'outline'} aria-pressed={mode === view} data-count={view === 'practice' ? practiceCount : view === 'coverage' ? entry.requirements.length : view === 'sources' ? n.current.sources.length : undefined} onClick={() => setMode(view)}>{view === 'study' ? entry.goal === 'assessment' ? 'Assessment prep' : entry.goal === 'assignment' ? 'Assignment workspace' : 'Study guide' : view === 'practice' ? 'Practice' : view === 'coverage' ? 'Coverage' : 'Sources'}</Button>)}</nav>
    <NotebookPackageView mode={draft ? 'all' : mode} pkg={draft ?? n.current} entryId={n.entryId} change={draft ? updateText : undefined} progress={n.progress} onProgress={progress} reader={!draft} />
  </section>
}
