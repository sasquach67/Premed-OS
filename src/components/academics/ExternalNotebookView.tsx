...
function BlockView({ block, path, change, pkg, progress, onProgress, reader = false, inlineSources = false, onShowEvidence }: { block: NotebookBlock; path: (string | number)[]; change?: ChangeText; pkg: NotebookPackage; progress?: NotebookProgress; onProgress?: (id: string, response: string, complete: boolean) => void; reader?: boolean; inlineSources?: boolean; onShowEvidence?: (id: string) => void }) {
  ...
  return <div className={`en-block en-block-${block.type}${block.type === 'practice' && !change ? ' en-practice-unit' : ''}`}>
    ...
    {block.type === 'practice' && (change ? <>{text(block.prompt, 'prompt', 'Practice prompt')}<NotebookPracticeStimulus block={block} />{text(block.answer, 'answer', 'Answer')}{text(block.rationale, 'rationale', 'Explanation')}<EvidenceView evidence={block} pkg={pkg} /></> : <NotebookPracticeCard
      ...
      origin={practiceDisplay?.label ?? (block.provenance === 'source' ? 'Question from supplied course material' : 'Additional practice question')}
      prompt={text(practiceDisplay?.body ?? block.prompt, 'prompt', 'Practice prompt')} stimulus={<NotebookPracticeStimulus block={block} />} hasStimulus={Boolean(block.stimulusBlockIds?.length)}
      answer={text(block.answer, 'answer', 'Answer')} rationale={text(block.rationale, 'rationale', 'Explanation')} original={block.prompt} missing={missing}
      sources={reader ? <ReaderSources blocks={[block]} pkg={pkg} scope="item" inlineSources={practiceSources} onInlineSources={() => setPracticeSources(previous => !previous)} /> : <EvidenceView evidence={block} pkg={pkg} />}
      earlierWork={onProgress && <details className="nbr-earlier-work"><summary>Your earlier work on this item</summary><p className="en-text">{work.response || 'No saved response.'}</p><p>{work.complete ? 'Previously marked explainable without looking.' : 'Not previously marked explainable.'}</p></details>}
    />)}
    ...
    {reader ? block.type !== 'practice' && block.type !== 'worked-example' && <div className="nbr-block-foot" hidden={!inlineSources}><button type="button" className="nbr-cite" onClick={() => onShowEvidence?.(block.id)}>{block.provenance.replaceAll('-', ' ')} / {block.excerptIds.length} excerpts</button></div> : block.type !== 'practice' && <><small className="en-muted">{block.provenance.replaceAll('-', ' ')}</small>{block.type !== 'worked-example' && <EvidenceView evidence={block} pkg={pkg} />}</>}
  </div>
}
...
export function ExternalNotebookView({ lecture, courseCode, onNavigateEntry }: { lecture: LectureRecord; courseCode: string; onNavigateEntry?: (id: string) => void }) {
  ...
  if (updating && n.updateSession) return <section className="external-notebook" aria-label="Update saved notebook"><div className="en-actions"><Button variant="outline" onClick={() => { setUpdating(false); setMode('study') }}>Back to saved entry</Button><Button variant="ghost" onClick={() => startUpdate(true)}>Restart from latest saved entry</Button></div><p className="en-muted">Restart only when you need a newer baseline. Keep any unfinished prompt or proposal first; it starts a fresh update draft, not a new notebook.</p><p role="status">{message}</p><NotebookUpdateImageFiles lecture={lecture} revision={n.updateSession} disabled={staleUpdate} /><ExternalNotebookWorkflow key={n.updateSession.id} courseId={lecture.courseId} revision={n.updateSession} baselineFresh={!staleUpdate} onImported={id => { setUpdating(false); setMode('study'); setMessage(id === lecture.id ? 'Notebook update saved. Previous versions remain in history.' : 'Separate entry saved. The original notebook remains unchanged.'); onNavigateEntry?.(id) }} /></section>
  ...
  return <section className="external-notebook nbr" aria-label="Saved external notebook"><header className="en-header nbr-head"><div className="nbr-head-main"><p className="en-eyebrow">Saved in {courseCode}</p><h1>{lecture.title}</h1><p className="nbr-scope en-text">{entry.scope}</p><details className="nbr-notebook-details"><summary>Notebook details</summary><p>{entry.goal} / Revision {entry.revision}{entry.baseRevision ? ` / Based on revision ${entry.baseRevision}` : ''}</p><p>{entry.sections.length} sections / {practiceCount} practice items / {entry.objectives.length} objectives / {n.current.sources.length} sources</p>{n.editedAt && <p>Updated locally</p>}</details></div><div className="en-actions nbr-head-actions">{(practiceCount > 0 || entry.objectives.length > 0) && <Button onClick={() => setMode('practice')}>Practice recall</Button>}<Button variant="outline" disabled={Boolean(draft)} onClick={() => { setDraft(structuredClone(n.current)); setEditBase(notebookContentKey(n)); setMessage('Editing a separate copy. Save to keep changes; the previous content stays in history.') }}>Edit entry</Button><Button variant="outline" onClick={() => startUpdate()}>Update this notebook</Button></div></header>
    <details className="nbr-tools"><summary>Downloads, notes and history</summary><NotebookImportAdjustments raw={n.acceptedRaw ?? n.originalRaw} /><div className="nbr-tools-body"><NotebookPortableExports lecture={lecture} />
    ...
    <details className="en-small-detail"><summary>JSON-only exports</summary><div className="en-actions"><Button variant="outline" onClick={() => { try { downloadNotebookText(notebookExportFilename(n, 'current'), exportNotebook(lecture, 'current')) } catch (error) { setMessage((error as Error).message) } }}>Export current JSON</Button><Button variant="outline" onClick={() => downloadNotebookText(notebookExportFilename(n, 'original'), exportNotebook(lecture, 'original'))}>Export original</Button><Button variant="outline" onClick={() => downloadNotebookText(notebookExportFilename(n, 'backup'), exportNotebook(lecture, 'backup'))}>JSON records with progress</Button></div><p>These JSON files do not contain image bytes. Use the complete portable backup to move or preserve a visual notebook.</p></details>
    ...
    {draft && <div className="en-notice"><b>Editing your copy</b><p>Exports use the last saved content. Reveal collapsed sections to edit their text.</p><p>{notebookPracticePolicy(n.current, draft, n.entryId).explanation}</p><div className="en-actions"><Button onClick={save}>Save edits</Button><Button variant="outline" onClick={() => { setDraft(null); setMode('study'); setNotes(n.notes); setNotesBase(n.notes); setMessage('Unsaved content changes discarded.') }}>Cancel edits</Button></div></div>}
    <p className={message ? 'en-reader-status' : 'sr-only'} role="status" aria-live="polite">{message}</p>
    <nav className="en-actions nbr-views" aria-label="Notebook reading views">{(['study', 'practice', 'sources'] as const).map(view => <Button asChild key={view} variant={mode === view ? 'default' : 'outline'} aria-pressed={mode === view} data-count={view === 'practice' ? practiceCount : view === 'sources' ? n.current.sources.length : undefined} onClick={() => setMode(view)}><button type="button">{view === 'study' ? entry.goal === 'assessment' ? 'Assessment prep' : entry.goal === 'assignment' ? 'Assignment workspace' : 'Study guide' : view === 'practice' ? 'Practice' : 'Sources'}</button></Button>)}</nav>
