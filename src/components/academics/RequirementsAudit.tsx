...
function PlanningLibraryContext({ context, courseCodes, onChange }: { context: import('@/lib/types').PlanningProgramContext; courseCodes: string[]; onChange: (patch: Partial<import('@/lib/types').PlanningProgramContext>) => void }) {
  ...
  return <section className={cn('rounded-[18px] border p-5', planningSurface)}>
    ...
    <div className="mt-4 grid max-w-4xl gap-3 sm:grid-cols-2">
      <label className="text-xs font-extrabold text-[#6a5f54] dark:text-[#d0c3b5]">Degree / track<SelectField aria-label="Degree / track" value={selectedProgramId ?? ''} onValueChange={(value) => onChange({ selectedProgramId: value || undefined })} className={cn('mt-1 h-10 w-full rounded-[10px] border px-3 text-sm font-bold', planningInset)} options={[{ value: '', label: 'Select a catalog program' }, ...sortedPlanningPrograms.map((set) => ({ value: set.id, label: planningProgramLabel(set) }))]} /></label>
      ...
      <label className="text-xs font-extrabold text-[#6a5f54] dark:text-[#d0c3b5]">Program admission status<SelectField aria-label="Program admission status" value={programAdmissionStatus} onValueChange={(value) => onChange({ programAdmissionStatus: value as import('@/lib/types').PlanningProgramContext['programAdmissionStatus'] })} className={cn('mt-1 h-10 w-full rounded-[10px] border px-3 text-sm font-bold', planningInset)} options={[{ value: 'not-applicable', label: 'Not admission-gated' }, { value: 'planning', label: 'Planning only' }, { value: 'applied', label: 'Application submitted' }, { value: 'admitted', label: 'Admission recorded locally' }]} /></label>
