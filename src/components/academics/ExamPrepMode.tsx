...
interface ExamPrepModeProps {
  ...
  onExit: () => void
  onOpenTab: (tab: 'materials' | 'assignments') => void
}
...
export function ExamPrepMode({ course, data, exam, onExit, onOpenTab }: ExamPrepModeProps) {
  ...
  const selectedFileIds = new Set(exam.examStudyFileIds ?? [])
  const mockChunks = data.sourceChunks.filter((chunk) => chunk.courseId === course.id && selectedFileIds.has(chunk.fileId))
  const mockDormant = fullMockDormantReasons(exam, [], mockChunks)
  function toggleExamMaterial(fileId: string) {
    update((draft) => {
      const saved = draft.academics.classCenter.assignments.find((item) => item.id === exam.id)
      if (!saved) return
      const selected = saved.examStudyFileIds ?? []
      saved.examStudyFileIds = selected.includes(fileId)
  ...
  }), [course.id, data.assignments, data.files, draftIntensity, exam])
  ...
  async function startMock() {
    ...
    try {
      ...
      update((draft) => {
        ...
        attempts.unshift(startGeneratedMock({ id: uid(), courseId: course.id, examAssignmentId: exam.id, topicIds: [], sourceChunkIds: mockChunks.map((chunk) => chunk.id), specId: 'class-full-mock-v1', specHash: outcome.specHash!, questions: outcome.questions!, startedAt: Date.now() }))
      })
      setOpeningMock(true)
      window.setTimeout(() => {
  ...
  return <main className="min-h-full bg-background" aria-labelledby="exam-prep-title">
    ...
    <div className="mx-auto grid max-w-6xl gap-5 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1fr)_18rem] lg:px-8">
      ...
      <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
        ...
        <Card><CardHeader><CardTitle className="text-lg">Exam materials</CardTitle></CardHeader><CardContent className="space-y-2 text-sm text-muted-foreground"><p>Choose the sources this exam covers. New plans and practice mocks use only these selections.</p>{courseFiles.map((file) => <label key={file.id} className="flex items-start gap-2"><input type="checkbox" checked={selectedFileIds.has(file.id)} onChange={() => toggleExamMaterial(file.id)} aria-label={`Include ${file.title} for ${exam.title}`} /><span>{file.title}</span></label>)}{!courseFiles.length && <p>No class materials added yet.</p>}<Button size="sm" variant="outline" onClick={() => onOpenTab('materials')}><BookOpen className="size-4" /> Open materials</Button></CardContent></Card>
        <Card><CardHeader><CardTitle className="text-lg">Full mock</CardTitle></CardHeader><CardContent className="space-y-3 text-sm text-muted-foreground">{mockDormant.length ? <><p>Not ready yet: {mockDormant.map((key) => key === 'exam-scope' ? 'selected exam materials' : key === 'study-material' ? 'linked study materials' : key.replaceAll('-', ' ')).join(', ')}.</p><Button size="sm" variant="outline" onClick={() => onOpenTab('materials')}>Resolve missing evidence</Button></> : <><p>Generated only from this exam’s linked student material. It is not a professor or past exam.</p>{mockGenerationPhase !== 'idle' && <GenerationProgress phase={mockGenerationPhase} outputLabel="Full practice mock" errorMessage={mockGenerationError} steps={[{ label: 'Prepare scope', detail: 'Checking this exam’s linked material' }, { label: 'Write practice set', detail: 'Keeping every question source-grounded' }, { label: 'Save attempt', detail: 'Opening your new practice set' }]} />}<Button size="sm" onClick={startMock} disabled={mockBusy}>{mockBusy ? 'Building…' : 'Start full mock'}</Button></>}{completedMock && <p className="border-t border-border pt-3 text-xs">A completed attempt is available below for source-based review.</p>}</CardContent></Card>
      </aside>
    </div>
  </main>
}
...
function MockAutopsy({ attempt, data, onOpenTab }: { attempt: GeneratedMockAttempt; data: ClassCenterData; onOpenTab: ExamPrepModeProps['onOpenTab'] }) {
  ...
  return <Card className="border-primary/20"><CardHeader><CardTitle className="text-lg">Post-mock autopsy</CardTitle><p className="mt-1 text-sm text-muted-foreground">Only unanswered or self-flagged evidence appears here. This is not a score or exam forecast.</p></CardHeader><CardContent className="space-y-3">
    ...
    {actionable.map((question) => {
      ...
      return <div key={question.id} className="grid gap-3 rounded-[13px] border border-border bg-muted/20 p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><p className="font-bold">Question evidence</p><Badge variant={unanswered ? 'warning' : 'outline'}>{unanswered ? 'Unanswered' : 'Self-flagged'}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{question.prompt}</p><p className="mt-2 text-xs font-semibold text-primary">Source: {file?.title ?? 'Selected course material'}</p></div><div className="flex flex-wrap gap-2">{file && <Button size="sm" variant="outline" onClick={() => onOpenTab('materials')}><BookOpen className="size-4" /> Open source</Button>}</div></div>
    })}
  </CardContent></Card>
}
...
function PlanPreview({ preview, onOpenTab, onCreate, capacityReady }: { preview: ReturnType<typeof buildExamPrepPlan>; onOpenTab: ExamPrepModeProps['onOpenTab']; onCreate: () => void; capacityReady: boolean }) {
  ...
  if (!preview.plan.items.length) return <div className="rounded-xl border border-dashed border-border p-4 text-sm text-muted-foreground"><p className="font-bold text-foreground">Nothing attributable to schedule yet.</p><p className="mt-1">Choose exam materials first, then build this plan from them.</p><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => onOpenTab('materials')}>Open materials</Button></div></div>
...
function PlanRows({ plan, data, onOpenTab, onPatch }: { plan: ExamPrepPlan; data: ClassCenterData; onOpenTab: ExamPrepModeProps['onOpenTab']; onPatch: (id: string, patch: Partial<ExamPrepPlanItem>) => void }) {
  ...
  const itemLabel = (item: ExamPrepPlanItem) => item.owner === 'manual' ? item.manualLabel ?? 'Manual task' : item.owner === 'topic' ? data.topics.find((topic) => topic.id === item.topicId)?.title ?? 'Saved study task' : item.owner === 'assignment' ? data.assignments.find((assignment) => assignment.id === item.assignmentId)?.title ?? 'Removed assignment' : data.files.find((file) => file.id === item.fileId)?.title ?? 'Removed material'
  const ownerTab = (item: ExamPrepPlanItem) => item.owner === 'assignment' ? 'assignments' : 'materials'
  if (!plan.items.length) return <p className="rounded-xl border border-dashed border-border p-4 text-sm text-muted-foreground">This plan has no recorded scope yet. Add a manual task or select exam materials.</p>
  return <div className="space-y-4">{Object.entries(groups).sort(([a], [b]) => a.localeCompare(b)).map(([date, items]) => <div key={date}><div className="mb-2 flex items-center justify-between"><p className="font-display font-extrabold">{date === today ? 'Today' : dateLabel(date)}</p><p className="text-xs font-semibold text-muted-foreground">Finish time not set</p></div><div className="space-y-2">{items.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-xl border border-border bg-muted p-3"><input type="checkbox" className="size-4 accent-primary" checked={item.state === 'complete'} onChange={(event) => onPatch(item.id, event.target.checked ? { state: 'complete', completedAt: Date.now() } : { state: 'planned', completedAt: undefined })} aria-label={`Mark ${itemLabel(item)} complete`} /><div className="min-w-0 flex-1"><p className={cn('font-bold', item.state !== 'planned' && 'text-muted-foreground line-through')}>{itemLabel(item)}</p><p className="mt-0.5 text-xs text-muted-foreground">{item.owner === 'manual' ? 'Manual task' : 'From this class'}</p></div>{item.owner !== 'manual' && item.owner !== 'topic' && <Button size="sm" variant="ghost" onClick={() => onOpenTab(ownerTab(item))}>Open</Button>}</div>)}</div></div>)}</div>
}
