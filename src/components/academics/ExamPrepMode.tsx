...
import { ArrowLeft, CheckCircle2, ListChecks, Plus, RotateCcw, Target } from 'lucide-react'
import type { ClassAssignment, ClassCenterData, Course, ExamPrepIntensity, ExamPrepPlan, ExamPrepPlanItem } from '@/lib/types'
  ...
  return <main className="min-h-full bg-background" aria-labelledby="exam-prep-title">
    ...
    <div className="mx-auto grid max-w-6xl gap-5 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1fr)_18rem] lg:px-8">
      ...
      <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
        <Card><CardHeader><CardTitle className="text-lg">Pace</CardTitle></CardHeader><CardContent className="space-y-2">{(['accelerated', 'steady'] as const).map((intensity) => <button key={intensity} type="button" onClick={() => setIntensity(intensity)} className={cn('w-full rounded-xl border p-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring', (existing?.intensity ?? draftIntensity) === intensity ? 'border-primary bg-primary/10' : 'border-border bg-muted/20 hover:bg-muted/45')}><span className="font-extrabold">{paceCopy[intensity].label}</span><span className="mt-1 block text-xs leading-relaxed text-muted-foreground">{paceCopy[intensity].detail}</span></button>)}</CardContent></Card>
