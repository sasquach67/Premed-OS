...
import { BookOpen, Brain, ChevronDown, CircleHelp, ClipboardPaste, FilePlus2, FileStack, FileUp, Link2, ListChecks, NotebookText, Search, Sparkles } from 'lucide-react'
import type { ClassCenterData, Course, LectureRecord, SourceChunk } from '@/lib/types'
...
export function LectureCapturePanel({ courseId, course, data, onOpenNotes, initialLectureId, initialDestination = 'overview' }: { courseId: string; course?: Pick<Course, 'code' | 'title'>; data: ClassCenterData; onOpenNotes: () => void; initialLectureId?: string; initialDestination?: LectureDestination }) {
    ...
    <Card className="lecture-capture-panel overflow-hidden border-border bg-card shadow-[0_10px_26px_-14px_rgba(0,0,0,0.55)]">
      ...
      <CardContent className="p-4">
        ...
        {view === 'review' && activeLecture && <>
          ...
          />
          {artifact && <div className="mt-4 border-t border-border pt-4"><MaterialGenerationIntake artifact={artifact} courseId={courseId} courseLabel={`Lecture ${lectureNumber(activeLecture.id)}`} course={course} files={activeLectureFiles} lectureId={activeLecture.id} onClose={() => setArtifact(null)} /></div>}
        </>}
