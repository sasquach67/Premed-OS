...
export function LectureCapturePanel({ courseId, course, data, onOpenNotes, initialLectureId, initialDestination = 'overview', displayMode = 'dialog', onNavigateLecture, onDeletedLecture }: {
  ...
  if (activeLecture?.importedNotebook) return <ExternalNotebookView key={activeLecture.id} lecture={activeLecture} courseCode={course?.code ?? courseId} onNavigateEntry={id => { setActiveLectureId(id); onNavigateLecture?.(id) }} />
...
function LectureWorkspace({ course, courseId, data, lectures, activeLecture, view, onView, onSelect, onDeleted, onRebuild, onOpenNotes, onHelp, help, embedded = false, standalone = false }: { course?: Pick<Course, 'code' | 'title'>; courseId: string; data: ClassCenterData; lectures: LectureRecord[]; activeLecture: LectureRecord; view: WorkspaceView; onView: (view: WorkspaceView) => void; onSelect: (lecture: LectureRecord) => void; onDeleted: (lectureId: string) => void; onRebuild: () => void; onOpenNotes: () => void; onHelp: () => void; help: ReactNode; embedded?: boolean; standalone?: boolean }) {
  ...
  const content = <><div className="mb-3 flex flex-wrap gap-2"><GenerationUsageDialog /><StudyPackageImportDialog courseId={courseId} onImported={id => { const imported = useStore.getState().academics.classCenter.lectures.find(l => l.id === id); if (imported) onSelect(imported) }} /></div>{activeLecture.importedStudyPackage && <p role="status" className="mb-3 rounded-lg border bg-muted p-3 text-sm">Imported guide · externally created, not independently reviewed. Sources are supplied excerpts; their presence does not verify the claims.</p>}{view === 'brief' && activeLecture.studyGuide && !activeLecture.importedStudyPackage && <GenerationReviewNotice status={activeLecture.generationAuditStatus} />}{activeLecture.studyGuide && activeLecture.processingError && <p role="status" className="mb-4 rounded-lg border border-border bg-muted p-3 text-sm text-muted-foreground">{activeLecture.processingError}</p>}{view === 'brief' && (activeLecture.studyGuide
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    <section aria-labelledby="generated-lecture-materials">
      ...
      <div className="mt-3 space-y-2">
        {lecture.studyGuide && <GeneratedMaterialRow icon={BookOpen} title={lecture.importedStudyPackage ? 'Imported guide' : lecture.notebookOutput === 'tailored-page' ? 'Notebook page' : 'Study Guide'} detail={`${lecture.studyGuide.sections.length} sections · At a glance through full depth · source traced`} onOpen={onOpenBrief} />}
