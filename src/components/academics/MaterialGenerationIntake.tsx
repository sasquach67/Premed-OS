    ...
    <Card className="border-border bg-card shadow-[0_16px_36px_-20px_rgba(0,0,0,0.62)]">
      <CardContent className="p-4 sm:p-5">
        ...
        {(artifact === 'unit-question-bank' || artifact === 'unit-mastery-outline') && <section className="mt-4 rounded-2xl border border-border bg-muted/30 p-3.5" aria-label="Resource scope settings">
          <div className="flex flex-wrap items-end justify-between gap-3"><div className="min-w-[14rem] flex-1"><p className="font-display text-sm font-extrabold">{classType === 'writing' ? 'Assignment or reading scope' : 'Lecture, unit, or exam scope'}</p><p className="mt-1 text-xs font-semibold text-muted-foreground">{classType === 'stem' ? 'Name the lecture, unit, or exam scope. The Mastery Map is the standard map; the question bank uses it without duplicating the resource.' : 'Name the part of the course this resource should cover. Syllabus objectives stay primary; selected readings and class material provide the evidence.'}</p><div className="mt-3 flex gap-2">{artifact === 'unit-mastery-outline' && <label className="text-xs font-extrabold"><span className="sr-only">Mastery Map scope type</span><SelectField aria-label="Mastery Map scope type" className="h-full bg-card text-sm" value={masteryScope} onValueChange={value => setMasteryScope(value as 'lecture' | 'unit' | 'exam')} options={[{ value: 'lecture', label: 'Lecture' }, { value: 'unit', label: 'Unit' }, { value: 'exam', label: 'Exam' }]} /></label>}<label className="block min-w-0 flex-1 text-xs font-extrabold"><span className="sr-only">Resource scope label</span><input className="w-full rounded-lg border border-border bg-card px-3 py-2 text-sm font-semibold" value={unitLabel} onChange={(event) => setUnitLabel(event.target.value)} placeholder={classType === 'writing' ? 'Essay 1 · Sources and evidence' : classType === 'general' ? 'Week 4 · Global institutions' : 'Unit 2 · Transcription and translation'} /></label></div>{artifact === 'unit-question-bank' && masteryOutlines.length > 0 && <p className="mt-2 text-xs font-semibold text-muted-foreground">{matchingMasteryOutline ? `Mastery Map linked · ${matchingMasteryOutline.standards.length} syllabus standards covered.` : 'Create or select a matching Mastery Map first for a closed standard-coverage check.'}</p>}</div>{artifact === 'unit-question-bank' && <label className="flex items-center gap-2 text-xs font-extrabold"><span>Current scope</span><SelectField aria-label="Current scope" className="bg-card" value={String(currentUnitPercent)} onValueChange={value => setCurrentUnitPercent(Number(value))} options={[{ value: '100', label: '100%' }, { value: '70', label: '70% · 30% prior' }, { value: '50', label: '50% · 50% prior' }]} /></label>}</div>
        </section>}
