...
it('distinguishes minimum materials, optional lecture sources, partial exam scope and external checkpoints', async () => {
  ...
  await click('Next')
  expect(container.querySelector<HTMLTextAreaElement>('textarea[readonly]')!.value).toContain('notebook-instructions-beta-5')
  ...
  expect(container.textContent).toContain('show you a readable draft, and wait for your confirmation before making the notebook JSON')
  expect(container.textContent).toContain('review any substantive updates, then explicitly confirm')
