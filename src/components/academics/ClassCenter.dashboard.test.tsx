...
describe('Daily Class Center persisted dashboard boundary', () => {
  ...
  it('previews from the card body or keyboard and opens the full hub only from Open', async () => {
    ...
    const actionSlot = card!.querySelector<HTMLElement>('[data-testid="class-card-action-slot"]')
    const deadline = card!.querySelector<HTMLElement>('[data-testid="class-next-deadline"]')
    ...
    expect(hoverActions.className).toContain('md:inset-x-1')
    expect(hoverActions.className).toContain('md:top-1/2')
    expect(hoverActions.className).toContain('md:h-16')
