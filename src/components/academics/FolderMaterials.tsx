...
export function FolderMaterials({ courseId, courseLabel, onBack }: { courseId: string; courseLabel: string; onBack: () => void }) {
  ...
  return <section className="material-folder" aria-label="Connected class folder">
    ...
    {syncPending && <p role="status" className="mf-description">{syncNeedsAttention ? 'Account sync needs attention before connecting or changing files.' : `${cloud.progress || 'Checking your saved account…'} Folder connection will be available automatically when it finishes.`} <Link to="/settings">View sync status</Link></p>}
