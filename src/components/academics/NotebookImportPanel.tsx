...
export function NotebookImportPanel({ courseId, onImported }: { courseId: string; onImported: (id: string) => void }) {
  ...
  return <section className="external-notebook" aria-label="Import external notebook"><h2>Import into {course.code}</h2><p>Choose the JSON file from your AI, or paste its complete JSON block. Review the rendered result, then explicitly save it to this class.</p>
    <label className="en-field">Notebook JSON file<input type="file" accept=".json,application/json" disabled={busy} onChange={async event => {
    ...
    <label className="en-field">Paste complete JSON<textarea className="en-json" value={raw} disabled={busy} onChange={event => { clearPreview(); setRaw(event.target.value) }} placeholder="Paste the complete notebook package or fenced JSON block" /></label>
    <Button variant="outline" disabled={busy || !raw.trim()} onClick={() => void validate()}>{busy ? 'Checking package...' : 'Validate and preview'}</Button>
    ...
    {preview && <div aria-label="Validated notebook preview"><div className="en-notice"><b>Destination: {course.code} / {course.title} / {course.term}</b><p>{preview.package.entries.length} entries / {preview.package.sources.length} supplied sources</p><ul>{plan.map(item => { const oldRevision = item.previous?.importedNotebook?.original.entries.find(e => e.id === item.entry.id)?.revision; return <li key={item.entry.id}>{notebookEntryLabel(item.entry)}{item.duplicate ? ' / Already saved' : oldRevision !== undefined ? item.entry.revision > oldRevision ? ' / Newer revision' : item.entry.revision < oldRevision ? ' / Older revision' : ' / Changed content at the same revision' : ' / New entry'}</li> })}</ul></div>
      ...
      <NotebookPackageView pkg={preview.package} />
      <div className="en-actions"><Button disabled={busy || (wrongCourse && !destination) || (revised && !revisions)} onClick={() => void save()}>{duplicates.length === plan.length ? 'Open existing saved entry' : `Save editable ${plan.length === 1 ? 'entry' : 'entries'} to ${course.code}`}</Button></div>
    </div>}
  </section>
}
