...
describe('lecture import and workspace', () => {
  ...
  it('uses only materials added to this lecture, without a class-wide source picker', async () => {
    ...
    expect(generationMocks.generateStudyGuide.mock.calls[0][0].primarySourceChunkIds).toContain('transcript-chunk')
    expect(generationMocks.generateUnitMasteryOutline.mock.calls[0][0].primarySourceChunkIds).toContain('transcript-chunk')
    ...
    await render(courseId, 'lecture', 'transcript')
    expect(container.textContent).toContain('Read to understand.')
    expect(container.textContent).toContain('AT A GLANCE')
    expect(container.querySelector('nav[aria-label="Study guide sections"]')).toBeTruthy()
    expect(container.textContent).toContain('Acquisition develops as repeated pairings')
    const sourceDetails = [...container.querySelectorAll('details')].filter((item) => item.querySelector('summary')?.textContent === 'Sources')
    expect(sourceDetails).toHaveLength(2)
    expect(sourceDetails.every((item) => !item.open)).toBe(true)
    expect(container.textContent).not.toContain('Verify ·')
    expect(container.textContent).not.toContain('Lecture Brief')
    expect(container.querySelector('[aria-label="Generated concept map"]')).toBeNull()
