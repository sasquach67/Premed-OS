...
3. **The photo is inlined as a data URI** so the mockup stays single-file. **The build should serve a responsive, compressed image** — an unoptimised full-size photo is the easiest way to wreck this page's load time.
2. **The reference used a serif and HQ cannot.** Baloo 2 and Nunito are locked. **The looseness is what replaces it.** Normalising the leading back to the app's 1.45 kills the page.
...
6. **Two links only — LinkedIn and email** (confirmed Andy, Aug 2026). TikTok and Instagram were in the reference and are **cut**; do not re-add them.
   ...
   - **⚠ Worth Andy's decision before launch:** `elephon08@gmail.com` is a personal address published on a public page, which means public scraping and spam. **A forwarding alias on the eventual domain (`hello@…`) is the normal fix**, and it also means the address on the page doesn't change if he ever wants to stop using this inbox.
7. **Portrait asset:** `05-public/assets/andy-portrait.jpg`, pre-cropped to 3:4 and biased toward the subject. **The real build should serve a responsive, compressed image** — a large unoptimised portrait is the single easiest way to wreck this page's load time.
