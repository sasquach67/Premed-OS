...
def run(root,out):
    ...
    for goal in ('review','assessment','assignment'):
        ...
        for name,fragment in focused_fragments:
            assert fragment in actual and prompt_methodology_errors(root,goal,actual.replace(fragment,''))
            results.append({'case':'reject-beta16-'+goal+'-'+name,'expected':'complete shared rules preserve the focused presentation/evidence correction without weakening the schema','passed':True})
