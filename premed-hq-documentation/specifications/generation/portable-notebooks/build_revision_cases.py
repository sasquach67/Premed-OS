...
''')
    receipt={'promptBuild':'notebook-instructions-beta-8','executed':False,'ratings':None,'notice':NOTICE,'files':{p.name:{'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'bytes':p.stat().st_size} for p in sorted(out.iterdir()) if p.is_file() and p.name!='case-manifest.json'}}
