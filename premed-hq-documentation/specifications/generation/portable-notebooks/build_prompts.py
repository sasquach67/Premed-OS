...
PROMPT_BUILD = 'notebook-instructions-beta-14'
...
def build(root, out):
    ...
    schema_path=GEN+'portable-notebooks/notebook-package-v3.schema.json'
    schema_raw=(root/schema_path).read_bytes(); schema=json.loads(schema_raw)
    schema_text=json.dumps(schema,ensure_ascii=False,separators=(',',':'))
    (out/'notebook-package-v3.schema.json').write_bytes(schema_raw)
...
'''
    ...
    contract={'instructionsVersion':'notebook-workflows-draft-3','formatVersion':3,'promptBuild':PROMPT_BUILD,'delivery':'Exactly three prewritten static goal templates, derived from canonical Markdown during development. Goal selection retrieves the chosen template; placeholder insertion and student notes are local plain-text operations. No AI API generates or rewrites prompts. The student uses the resulting prompt in their preferred AI to generate notebook content.','templates':{g:'copy-prompt-'+g+'.md' for g in goal_rules},'outputSchema':'notebook-package-v3.schema.json','acceptedLegacySchema':'notebook-package.schema.json','modes':MODE_CONFIG,'placeholders':dict(zip(TOKENS,KEYS)),'replacement':'Single-pass literal token replacement using JSON.stringify(value); never rescan replacement values. All values string or null; classPreferences defaults to empty string.','identity':'Preview text, clipboard text and UTF-8 .md download bytes come from the same composed string. Do not prepend, append, regenerate or normalize one surface separately.','defaults':{**{t:None for t in TOKENS},'CLASS_PREFERENCES':''},'courseProfiles':'No inferred class presets. Use the actual class identity, explicit student preferences, and supplied source evidence. These class values are the complete class-specific input; the five manual course projects are not inspected by this builder.'}
