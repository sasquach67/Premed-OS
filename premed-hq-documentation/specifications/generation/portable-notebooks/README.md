...
## External conversation contract — current beta 4
...
`conversation-examples.json` contains nine authored expected scenarios, rendered into EXPECTED-CONVERSATIONS.md. The five earlier scenarios are updated for the final confirmation gate; four additions show complete inputs stopping at review, tweaks then confirmation, visible partial multiweek approval and a normal chat without a project. Actual short hint drafts and a complete readable counterpart of the existing partial assessment fixture make review inspectable. Invalid/truncated repair samples remain in conversation-case-files/, outside valid-import globs. These files are not extra prompts or executed model trials; no ratings are prefilled.

Published beta-2 and beta-3 snapshots retain their original identities and hashes. Their optional preservation checks run when each archive is present (a fresh staging directory without both historical archives has two fewer checks). Beta-3 app verification does not establish the new beta-4 conversation behavior or asset parity. Coordinator-owned integration and real trials are separate acceptance steps.
