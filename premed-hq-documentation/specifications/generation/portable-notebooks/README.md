...
Published beta-2, beta-3 and beta-4 snapshots retain their original identities and hashes. Their optional preservation checks run when each archive is present (a fresh staging directory without the three historical archives has three fewer checks). Beta-3 app verification does not establish the new beta-4 conversation behavior or asset parity. Coordinator-owned integration and real trials are separate acceptance steps.
