...
| **D-6** | Anki export | **Yes — `.apkg`, schema designed for it now.** A premedOS note type carries hidden `concept_id`, `source`, and `spec` fields so round-trip regeneration survives. Scheduling state deliberately not exported | `04` §14 |
