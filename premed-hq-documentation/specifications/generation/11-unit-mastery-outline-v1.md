...
The runtime artifact spec is `src/lib/generation/artifacts/unitMasteryOutline.v1.ts`. Its structured response has `title`, `unit`, and `standards[]`; each standard has `id`, `title`, `freeRecallCues`, `understand`, `beAbleToDo`, `watchFor`, and `sourceChunkIds`.
