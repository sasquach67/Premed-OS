...
- **Milestones can spawn one implementation task.** The roadmap is direction on top; the task is action underneath. When a milestone comes due, the user can turn it into one real, linked Overview task. The milestone tracks *whether the pacing point is met*; the task tracks *the work*. **The task lands in Overview's own store** (§0, revised Aug 2026), which makes this simpler than it was: the spawning surface and the receiving surface are the same page. **This is not a Timeline checklist `step`:** authored `step`s remain Timeline-owned and flow into Overview → Soon as one record, two doors (§6.4). The implementation task is the student's separate concrete work item; it receives no default due date, priority, schedule, or completion state. Its link remains after completion, archive, or Trash so it can recover; v1 does not unlink or create a second task from the same milestone.
