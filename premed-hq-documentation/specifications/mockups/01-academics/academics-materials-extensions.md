...
> **Aug. 29, 2026 authority amendment — approved by Andy:** Review Session and Anki export remain retired. **Create study resources** opens one format menu for **Flashcards · Study Guide · Study Outline · Revised Notes**. Flashcards are generated only from selected class material and remain an inspectable Materials resource; they do not create an in-app review or scheduling path. Any conflicting historical section below is superseded.
...
| Study outputs | Select only student-supplied sources, then choose Flashcards, Study Guide, Study Outline, or Revised Notes. |
...
| Textbook excerpt | Let a student paste one bounded textbook section as a named, student-owned source for the existing resource formats. |
...
- **Revised Notes** is a distinct generated artifact. It begins from the student's selected notes, preserves their organization, language, and emphasis where the selected sources support them, and uses selected slides, transcript, or excerpts only to fill supported gaps or expose an unresolved source difference. If there is no student note selected, no Revised Notes artifact can begin: the student chooses notes or uses the current sources for Study Guide or Study Outline instead.
...
- **Study Outline** is a concise, objective-led organization of selected class evidence. It keeps source traces and does not create a review queue or readiness score.
- **Flashcards** are a source-backed generated class resource. They remain inspectable in Materials; Academics does not schedule them, review them, or send them to Anki.
...
- **Choose an output** is reached from the compact **Create study resources** dropdown. The contextual choice sheet shows the four distinct jobs without becoming a second generator home. On narrow screens it becomes a two-up and then one-column sequence without changing its order.
...
- A student starts from the artifact they want: **Flashcards**, **Study Guide**,
  **Study Outline**, or **Revised Notes**. The selected artifact stays visible
...
- A Study Guide, Study Outline, or Flashcard request uses only selected ready
  sources. Flashcards remain in Materials and never gain an Anki, in-app review,
  or scheduling path.
...
- **Flashcards resource** may read only the selected student-supplied class materials and keeps their provenance beside the deck. It remains an inspectable generated resource in Materials.
...
- No export or review handoff appears in Academics. The deck is read as a generated class resource alongside other selected-source outputs.
...
- These are nested states within the same Materials banner and quiet tool-row, not additional class tabs. The source map remains the grounding metaphor; after it, a four-choice artifact panel keeps each format legible without creating a separate generator page.
