...
**Status:** SUPERSEDED AS A STANDALONE PAGE · moved into
`Grades & Archive → Term report` on Aug 26, 2026 at Andy’s direction. The
integrated Variant A was approved by Andy on Aug 27, 2026.
**Source:** `academics-term-retrospective.html` · **Spec:** `tabs/01-academics.md` #43, §6.10-C

## Placement decision — Aug 26, 2026

The report is no longer registered as a separate Mockup Lab page. It now lives
as `term-retrospective` inside `academics-grades-archive.html`, where the saved
term record, GPA evidence, returned work, and rollover handoff already belong.
This standalone file remains only as the comparison record for the original
three treatments.
...
## Variant A review candidate — Term report

**For review.** Variant A is reframed as a
...
- 2026-08-23 — `fix(academics): route and align the end-of-term report` moved
  ...
  one-column reading composition. The mockup is **approved**; production
  promotion still requires its configured provider run to be proved end to end.

## Visual conformance sweep — 2026-08-26

- Aligned the banner to the exact blue/sage Academics layer and preserved the
  one-column report as a reading sequence rather than a generic card wall.
- Term-record evidence is now expressed as compact muted rows; observations
  use violet and carry-forward actions use sage, keeping interpretation and
  durable next steps visually distinct.
- Variants B and C remain comparison canvases only. Variant A is the
  **APPROVED** mockup target, including the honest too-little-to-say boundary.
