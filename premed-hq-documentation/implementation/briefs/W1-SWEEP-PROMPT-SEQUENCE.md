...
**Supersedes the earlier read-only version.** Andy, Aug 2026: *"the point of me doing this is so it can actually code stuff in from md that was previously added but before it was coded previously — so now I want to update it by coding the remaining parts in."*

**This prompt implements. It does not hand back a report to act on later.** The full text lives in `S-0-DRIFT-SWEEP.md` alongside this file — paste that.

**What it does:** greps every `.md` under `premed-hq-documentation/` for in-text dated rulings, dates the code with `git log` on `src/` (which is reliable, unlike `git log` on the docs), and implements every ruling that postdates the surface it governs.

**The boundary it must not cross:** no surface that has no approved mockup. A complete spec is not a design.

**Expect most of the grep hits to report OUT OF BOUNDARY.** The highest marker counts in the tree are `07-extracurriculars-board` (32), `03-clinical` (32), `05-shadowing` (28), `05-experience-pillar` (28) — pillars with no mockups and no code. The implementable drift is concentrated in `general.md`, `03-overview`, `00-product-shell`, `01-shared-interface-patterns`, `11-timeline-tasks`, `deferred.md`, and `architecture/02`.
