...
|---|---|---|---|
| Google Calendar Hero | **Code narrowed to `calendar.events.owned.readonly`; Google Cloud scope alignment and public OAuth verification remain pending** | A student who completes the Google test/consent flow can see timed events from the Primary calendar. The Hero does not request CalendarList, merge secondary/subscribed calendars, or show all-day events. | Google Cloud Data Access must exactly match the runtime scope, then the end-to-end consent and event display must be re-recorded for verification. See `implementation/google-calendar-oauth-verification.md`. |
