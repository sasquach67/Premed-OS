...
|---|---|---|---|---|
| S0 | **Long-horizon durability, D1 + D5** | `src/store/` | — | **Highest priority of anything in this file.** D1: `setItem` has no `try`/`catch`, so a quota-exceeded write is silently lost at save time. D5: migrations are tested per-step, not as a full chain from the oldest supported shape. **These two are the only items where the damage is unrecoverable.** Owned by `implementation/long-horizon-durability.md`; that file wins. |
...
| S6 | **Give tasks full functionality on Overview, narrow Timeline to the roadmap** | 9 files in `src/`, `Timeline.tsx` | **briefed** | **Ready to hand off: `implementation/briefs/S6-tasks-to-overview.md`.** *(Two earlier versions of this row were wrong. "Four links" was counted from the shell spec's as-built paragraphs, not the code — it is **19 references across 9 files** plus three of Timeline's four tabs. Then I said Overview must gain editing "before Timeline is stripped, or the app loses task editing" — **also wrong: there is no task editing to lose.**)* **The finding: a task cannot be renamed anywhere in the app.** Title is set once at quick-add; `deadline` has no editor; `notes` and `fileUrl` have no surface at all. Timeline's kanban has zero edit affordances and its `Add task` makes an untitled task you cannot name. **The seven "edit" links point at a page that cannot do the thing either.** Scope is now set by Andy's bar — *"even though it's a little widget, it should be fully functional"* — against a field-by-field parity table on `TaskItem`. **Does not depend on S7.** |
